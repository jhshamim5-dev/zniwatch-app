...
const workerProxy = {
  async fetch(request: Request): Promise<Response> {
    ...
    // Ensure the path is something like /api/proxy (adapted for Express routing)
...
export async function handleProxyRequest(req: IncomingMessage, res: ServerResponse): Promise<boolean> {
  if (!req.url?.startsWith('/api/proxy')) return false;
  ...
  if (req.method !== 'GET' && req.method !== 'HEAD') {
    // For completeness, though we only expect GET
    requestInit.body = Readable.toWeb(req as unknown as Readable) as unknown as ReadableStream;
    (requestInit as any).duplex = 'half';
  }

  const fetchReq = new Request(fullUrl, requestInit);

  try {
    const fetchRes = await workerProxy.fetch(fetchReq);
    ...
    if (fetchRes.body) {
      const nodeStream = Readable.fromWeb(fetchRes.body as unknown as import('stream/web').ReadableStream);
      nodeStream.pipe(res);
    } else {
      res.end();
    }
